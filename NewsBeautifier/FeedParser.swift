...
class FeedParser: NSObject, NSXMLParserDelegate {
    ...
    func parser(parser: NSXMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        autoreleasepool {
            var b = false
            // parse depending on feed type
            if self.feedType == .Atom { b = true; self.parseEndOfAtomElement(elementName, qualifiedName: qName) }
            if self.feedType == .RSS1 { b = true; self.parseEndOfRSS1Element(elementName, qualifiedName: qName) }
            if self.feedType == .RSS2 { b = true; self.parseEndOfRSS2Element(elementName, qualifiedName: qName) }
            if !b { NSNotificationCenter.defaultCenter().postNotificationName(kHANDLE_FINISH_PARSING, object: nil) }
        }
    }
